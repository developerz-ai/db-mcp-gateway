...
- **The file set is the lock.** Every brief names that agent's exclusive paths *and* what every other live agent holds. The `CLAUDE.md` layer table is the natural cut — `src/transport/`, `auth/`, `authz/`, `tools/`, `exec/`, `audit/`, `config/`, `state/` each have one reason to change. An agent needing a file it doesn't own **stops and reports the collision** — never edits across the line, never negotiates peer-to-peer; you mediate. Errors are per-layer (`src/*/error{,s}.rs`), so they cut with the layer they belong to — but `src/main.rs` wiring and `config/example.yaml` are touched by nearly everything: give each to exactly one agent, or keep them.
...
- **Reserve an "unowned" bucket and expect to fill it mid-run.** The fix often lands where no slice covers: the `main.rs` wiring, a `config/example.yaml` field, a `src/state/` migration, the spec. A homeless finding is the one most likely to be quietly dropped — when a report says "the real fix is outside my set", assign it immediately, don't file it.
