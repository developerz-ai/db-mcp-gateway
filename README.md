...
- **Synchronous audit log** — user, SQL, reason, row count, duration, outcome. The write commits *before* the response is sent; if it fails, the request fails. Retained in the gateway's Postgres with a configurable TTL and an hourly pruner, with an optional `stdout` stream sink for feeding an existing log pipeline. Archive to object storage and syslog/OTLP streaming are [roadmap Phase 4](website/docs/initial-idea/11-roadmap.md), not shipped.
